...
export default function ResumeUpload({ locale }: { locale: Locale }) {
  ...
  return (
    <div className="grid gap-8 lg:grid-cols-[1.02fr_0.98fr]">
      <section className={`rounded-[2rem] border border-slate-200 bg-white p-6 shadow-lg sm:p-7 ${textAlign}`}>
        <div className="mb-6 space-y-2">
          <p className="text-sm uppercase tracking-[0.22em] text-slate-500">{dict.upload.eyebrow}</p>
          <h2 className="text-2xl font-semibold text-slate-900">{dict.upload.title}</h2>
          <p className="text-sm leading-6 text-slate-600">{dict.upload.description}</p>
        </div>
        ...
        <div className="space-y-5">
          <div>
            <div className="mb-2 flex items-center justify-between gap-3">
              ...
              <button
                ...
                className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-xs font-medium uppercase tracking-[0.18em] text-slate-700 transition hover:bg-slate-100"
              >
            ...
            <textarea
              ...
              autoCorrect="off"
              spellCheck={false}
              className="min-h-[300px] w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-4 text-sm text-slate-900 outline-none transition placeholder:text-slate-400 focus:border-cyan-400 focus:ring-2 focus:ring-cyan-400/20"
            />
            <p className="mt-2 text-xs text-slate-500">{dict.upload.resumeHint}</p>
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              autoCorrect="off"
              spellCheck={false}
              className="min-h-[180px] w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-4 text-sm text-slate-900 outline-none transition placeholder:text-slate-400 focus:border-violet-400 focus:ring-2 focus:ring-violet-400/20"
            />
          </div>
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap">
            <button
              ...
              className="inline-flex items-center justify-center rounded-2xl bg-slate-900 px-5 py-3 text-sm font-medium text-white transition hover:bg-slate-800 disabled:cursor-not-allowed disabled:bg-slate-300 disabled:text-slate-500"
            >
            ...
            <button
              ...
              disabled={!canAnalyze || loadingAction !== null}
              className="inline-flex items-center justify-center rounded-2xl border border-cyan-200 bg-cyan-50 px-5 py-3 text-sm font-medium text-cyan-700 transition hover:bg-cyan-100 disabled:cursor-not-allowed disabled:border-slate-200 disabled:bg-slate-50 disabled:text-slate-400"
            >
            ...
            <button
              ...
              className="inline-flex items-center justify-center rounded-2xl border border-violet-200 bg-violet-50 px-5 py-3 text-sm font-medium text-violet-700 transition hover:bg-violet-100 disabled:cursor-not-allowed disabled:border-slate-200 disabled:bg-slate-50 disabled:text-slate-400"
            >
      ...
      <section className="space-y-6">
        {analysis ? (
          ...
          <div className={`rounded-[2rem] border border-dashed border-slate-300 bg-slate-50 p-8 text-sm leading-7 text-slate-600 ${textAlign}`}>
            {dict.upload.emptyState}
          </div>
        )}

        {rewrittenResume ? (
          <div className={`rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm ${textAlign}`}>
            <div className="mb-3 flex items-center justify-between gap-4">
              <h3 className="text-lg font-semibold text-slate-900">{dict.upload.atsRewrite}</h3>
              <span className="text-xs uppercase tracking-[0.2em] text-cyan-600">{dict.upload.plainText}</span>
            </div>
            <pre className="overflow-x-auto whitespace-pre-wrap rounded-2xl bg-slate-50 p-4 text-sm leading-7 text-slate-700">
              {rewrittenResume}
            </pre>
          </div>
        ) : null}
        ...
        {coverLetter ? (
          <div className={`rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm ${textAlign}`}>
            <div className="mb-3 flex items-center justify-between gap-4">
              <h3 className="text-lg font-semibold text-slate-900">{dict.upload.coverLetterTitle}</h3>
              <span className="text-xs uppercase tracking-[0.2em] text-violet-600">{dict.upload.generated}</span>
            </div>
            <pre className="overflow-x-auto whitespace-pre-wrap rounded-2xl bg-slate-50 p-4 text-sm leading-7 text-slate-700">
              {coverLetter}
            </pre>
